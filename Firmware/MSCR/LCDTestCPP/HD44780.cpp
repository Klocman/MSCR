...
void HD44780::lcd_string(const char *text)
{
	char c;
    while ( (c = *text++) )  lcd_char(c);
}
/*
void HD44780::lcd_fullscreen_string(const char *text)
{
	lcd_clrscr();
	lcd_home();
	char c;
...
void HD44780::lcd_string_format(const char *szFormat, ...)
{	
...
void HD44780::lcd_fullscreen_string_format(const char *szFormat, ...)
{
	char szBuffer[64]; //in this buffer we form the message // TODO: reduce size
	va_list pArgs;
	va_start(pArgs, szFormat);
	vsprintf(szBuffer, szFormat, pArgs);
	va_end(pArgs);
...
*/
#include <stdlib.h>
void HD44780::lcd_string_P(const char *szFormat)
{
	char *buffer;
	buffer=(char *)malloc(strlen_P(szFormat)+1);
	strcpy_P(buffer,szFormat);
	lcd_string(buffer);
	free(buffer);
}

void HD44780::lcd_string_format_P(const char *szFormat, ...)
{
	char szBuffer[64]; //in this buffer we form the message // TODO: reduce size
	va_list pArgs;
	va_start(pArgs, szFormat);
	vsprintf_P(szBuffer, szFormat, pArgs);
	va_end(pArgs);
...
void HD44780::lcd_cursor_mode( CursorModes mode )
{
	lcd_write(mode, 0); // Send instruction to make cursor invisible
}
